 ...
 * Usage:
		...
		$output_articles = '';
		$output_articles_post = simgo_get_hot_posts( get_option( 'posts_per_page' ) );
...
if ( !function_exists( 'simgo_get_hot_posts' ) ) {
	function simgo_get_hot_posts($limit){
		global $post;
		$limit = ( $limit == '' || !$limit ) ? 10 : $limit;	
		$new_args = array( 'orderby' => 'comment_count',
			'order' => 'desc',
