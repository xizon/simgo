	...
	class Featured_Image_Column {
		...
		function settings_page() {
			...
			<div class="wrap">
				...
				<h2><?php _e( 'Featured Image Column', 'simgo' ); ?>
				<small><?php printf( __( 'by <a href="%s">Austin Passy</a>', 'simgo' ), 'http://austin.passy.co' ); ?></small></h2>
						...
						<tbody>
							<tr valign="top">	
								<th scope="row" valign="top">
									<?php _e( 'Allowed Post Types', 'simgo' ); ?>
								</th>
								<td>								
									...
									<span class="description"><?php _e( 'By default all post types that support "thumbnails" are selected.', 'simgo' ); ?></span>
								</td>
							</tr>
						</tbody>
					</table>
		...
		function columns( $columns ) {
			...
			foreach( $columns as $key => $title ) {
				if ( $key == 'title' ) // Put the Thumbnail column before the Title column
					$new['featured-image'] = __( 'Image', 'simgo' );
